/* ═══════════════════════════════════════════════════════════════════════
     EUREKA · flowcharts · australian-design-application-procedure · v1
     ...
     CODE EMBED, per page — two lines:
       <div data-eureka="australian-design-application-procedure"></div>
       <script src="https://cdn.jsdelivr.net/gh/davidperkins2000/eureka@v0.3.10/flowcharts/australian-design-application-procedure.js"></script>
     ...
     ─── SPEC GRAMMAR ──────────────────────────────────────────────────
       Label | left annotation | right annotation
         ...
         << Target Step @left|@right      backward loop from this step
         ? tooltip prose                  cycled + shown on hover
     Fields after the label are optional; blank means omitted.
     Indentation attaches a line to the step above it. A `>>` target
     resolves FORWARD from its origin and a `<<` target resolves
     BACKWARD, so duplicate labels are safe in either direction.

     The BACKWARD LOOP is the one thing designs need that patents do not:
     an adverse report and a response cycle between each other until the
     grounds are resolved. Geometrically it is the same edge as a forward
     skip \u2014 out of one box, down the lane, into the other \u2014 so the layout
     and arrow code are untouched; only target resolution and the lane
     overlap test had to stop assuming from < to.
...
var SLUG = 'australian-design-application-procedure';
...
var SPEC = [
'Priority Design Application | 0 months',
...
'  ? Certification is optional at filing but required later if design is to be enforced.',
...
'    <svg role="img" aria-label="Australian registered design application process"',
'         aria-describedby="au-des-desc"></svg>',
'  </div>',
'</div>',
'<div class="eureka-flow-foot">',
'  <span class="eureka-eyebrow">',
'    <span class="eureka-dot" aria-hidden="true"></span>',
'    Australian Registered Design Application Process',
'  </span>',
'</div>',
'<div class="eureka-sr">',
'  <p id="au-des-desc">Procedural flowchart of an Australian registered design',
'    application from filing through a request for certification, examination',
'    and response, to certification and renewal at the fifth anniversary.</p>',
'</div>'
].join('\n');

var CLASSES = ['eureka', 'eureka--flow', 'eureka--au-design'];
...
function parse(src) {
  ...
  src.split('\n').filter(function (l) { return l.trim(); }).forEach(function (line) {
    ...
    if (t.charAt(0) === '?') {
    ...
    } else if (t.slice(0, 2) === '>>' || t.slice(0, 2) === '<<') {
      var back = t.charAt(0) === '<';
      ...
      skips.push({
        ...
        /* A loop reads more naturally on the left, where nothing else in
           this diagram sits; a forward skip still defaults right. */
        side: m ? m[2] : (back ? 'left' : 'right'),
        back: back
      });
    }
  });

  /* Directional search from the origin is what makes duplicate labels safe. */
  skips.forEach(function (s) {
    s.from = steps.findIndex(function (st) { return st.label === s.fromLabel; });
    if (s.back) {
      s.to = -1;
      for (var i = s.from - 1; i >= 0; i--) {
        if (steps[i].label === s.toLabel) { s.to = i; break; }
      }
    } else {
...
function assignLanes(skips, side) {
  /* A backward edge covers the same rows as the forward edge between the
     same two steps, so packing works on the NORMALISED span. Comparing
     raw from/to would give a loop a negative length and let it overlap
     anything. */
  function lo(s) { return Math.min(s.from, s.to); }
  function hi(s) { return Math.max(s.from, s.to); }

  var mine = skips.filter(function (s) { return s.side === side; })
    .sort(function (a, b) { return (hi(a) - lo(a)) - (hi(b) - lo(b)); });
  var lanes = [];
  mine.forEach(function (s) {
    var lane = -1;
    for (var i = 0; i < lanes.length; i++) {
      var clash = lanes[i].some(function (o) {
        return lo(s) < hi(o) && lo(o) < hi(s);
      });
